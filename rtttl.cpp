...
void RTTTL::init(const char *song) {
  int num;
  ...
  if (pgm_read_byte_near(p) == 'd') {
    p++;
    p++;              // skip "d="
    num = 0;

    while (isdigit(pgm_read_byte_near(p))) {
      num = (num * 10) + (pgm_read_byte_near(p++) - '0');
    }
  ...
  // get default octave
  if (pgm_read_byte_near(p) == 'o') {
    p++;
    p++;              // skip "o="
    num = pgm_read_byte_near(p++) - '0';
...
void RTTTL::loop() {
  if (delay && (millis() - time) < delay) {
    ...
    if (pgm_read_byte_near(p)) {
      ...
      // now, get scale
      if (isdigit(pgm_read_byte_near(p))) {
        scale = pgm_read_byte_near(p) - '0';
        p++;
      } else {
        scale = default_oct;
      }
      ...
      if (pgm_read_byte_near(p) == ',')
        ...
        if (note) {
          TV.tone(pgm_read_word_near(&notes[(scale - 5) * 12 + note]));
